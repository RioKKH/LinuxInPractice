...
{
    // timespec構造体を使って時間を計測する
    struct timespec before, after;
    // CLOCK_MONOTONICはシステム起動後の経過時間を返す
...
static void child_fn(int id, struct timespec *buf, int nrecord, unsigned long nloop_per_resol, struct timespec start)
{
    ...
    {
        struct timespec ts;
        // load()関数でCPUを占有する. この関数は引数で指定された回数だけループを回す
...
int main(int argc, char *argv[])
{
    ...
    if (total % resol)
    {
        fprintf(stderr, "<total>(%d) should be multipel of <resolution>(%d)\n", total, resol);
        exit(EXIT_FAILURE);
    }
